 ...
 */
class ResumeAlerts extends BaseDataModel
{
  use \FOF30\Model\Mixin\Assertions;
    ...
    // Not using convention for table names or primary key field
		$config['tableName'] = '#__cajobboard_resume_alerts';
    $config['idFieldName'] = 'resume_alert_id';

    // Define a contentType to enable the Tags behaviour
    $config['contentType'] = 'com_cajobboard.resume_alerts';
  ...
  /**
	 * Purge old resume alerts
	 */
	public function purgeExpiredResumeAlerts($duration)
	{
    // @TODO: Purge resume alerts that are older than $duration
  }
  ...
  protected function getKeywordsAttribute($value)
  {
    ...
    {
      throw new \Exception( 'Error in ResumeAlerts model while transforming keyword table field JSON to array: ' . json_last_error_msg() );
    }
  ...
  protected function setKeywordsAttribute($value)
  {
    ...
    if ( json_last_error() != JSON_ERROR_NONE )
    {
      throw new \Exception( 'Error in ResumeAlerts model while transforming keyword property array to JSON: ' . json_last_error_msg() );
    }

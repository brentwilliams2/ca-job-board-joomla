...
/**
 * Candidates data model SQL
 *
 * Uses schema https://schema.org/Candidate
 */
CREATE TABLE IF NOT EXISTS `#__cajobboard_candidates` (
  candidate_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Surrogate primary key', /* FK to #__cajobboard_ucm(id) */
  ...
  note VARCHAR(255) COMMENT 'A note to save with this candidate in the back-end interface.',
  ...
  description TEXT COMMENT 'Short description of the candidate, used for the text shown on social media via shares and search engine results.',

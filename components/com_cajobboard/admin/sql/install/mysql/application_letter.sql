...
/**
 * Application Letters data model SQL
 *
 * Uses schema https://calligraphic.design/schema/ApplicationLetter
 */
CREATE TABLE IF NOT EXISTS `#__cajobboard_application_letters` (
  application_letter_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Surrogate primary key', /* FK to #__cajobboard_ucm(id) */
  ...
  note VARCHAR(255) COMMENT 'A note to save with this application letter in the back-end interface.',
  ...
  description TEXT COMMENT 'Short description of the application letter, used for the text shown on social media via shares and search engine results.',

  /* SQL DDL */
  PRIMARY KEY (application_letter_id)
)
  ENGINE=innoDB

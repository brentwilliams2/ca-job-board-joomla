...
/**
 * Work Flow data model SQL
 *
 * Uses schema https://calligraphic.design/schema/WorkFlow
 */
CREATE TABLE IF NOT EXISTS `#__cajobboard_work_flows` (
  work_flow_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Surrogate primary key', /* FK to #__cajobboard_ucm(id) */
  ...
  note VARCHAR(255) COMMENT 'A note to save with this work flow in the back-end interface.',
  ...
  description TEXT COMMENT 'Short description of the work flow, used for the text shown on social media via shares and search engine results.',

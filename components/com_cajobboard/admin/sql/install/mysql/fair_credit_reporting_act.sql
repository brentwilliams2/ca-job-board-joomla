 ...
 */
 ...
 /**
 * Fair Credit Report Act table
 *
 * schema https://schema.org/Report
 */
CREATE TABLE IF NOT EXISTS `#__cajobboard_fair_credit_reporting_act` (
  fair_credit_reporting_act_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Surrogate primary key',
  ...
  /* SCHEMA: Thing */
  about BIGINT(20) UNSIGNED COMMENT 'The person the FCRA notice is sent to.', /* FK to #__cajobboard_persons */
  ...
  end_time DATETIME DEFAULT NULL COMMENT 'The date the signed FCRA notice was received.',
  start_time DATETIME DEFAULT NULL COMMENT 'The date the FCRA notice was sent to the job seeker for signing.',

  /* SCHEMA: Thing(potentialAction) -> Action(result) -> MediaObject */
  result__content_url VARCHAR(255) COMMENT 'System filename of the signed FCRA notice returned from the job seeker.',
  result__content_size BIGINT(20) UNSIGNED COMMENT 'File size of the signed FCRA notice in bytes.',

  /* SCHEMA: Thing(potentialAction) -> Action(result) -> CreativeWork */
  result__encoding_format CHAR(32) COMMENT 'MIME format of the document, e.g. application/pdf.',

  PRIMARY KEY (fair_credit_reporting_act_id)
)
  ENGINE=innoDB

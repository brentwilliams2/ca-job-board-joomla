 ...
 */
CREATE TABLE IF NOT EXISTS `#__cajobboard_email_message_templates` (
  email_message_template_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Surrogate primary key', /* FK to #__cajobboard_ucm(id) */
  ...
  /* SQL DDL */
  PRIMARY KEY (email_message_template_id)
)
  ENGINE=innoDB
...
REPLACE INTO `#__cajobboard_email_message_templates` (`email_message_template_id`, `name`, `description`, `subject`, `body`) VALUES
  ...
  (5, 'new_employer_review_received_notification', 'New employer review posted notification template', 'subject', 'body'),
  (6, 'job_post_alert', 'Job posting alert template', 'subject', 'body'),
  (7, 'resume_alert', 'Resume alert template', 'subject', 'body'),
  (8, 'new_message_received_notification', 'New user message notification template', 'subject', 'body'),
  (9, 'fair_credit_reporting_act_notice', 'FCRA notice sent when credit check is done', 'subject', 'body'),
  (10, 'new_application_received_notification', 'New application received for a job posting template', 'subject', 'body'),
  (11, 'gdpr_notice', 'GDPR user data removal notification template', 'subject', 'body'),
  (12, 'complete_job_seeker_profile_request', 'Reminders to complete a job seeker profile', 'subject', 'body'),
  (13, 'connectors_job_post_alert', 'Connectors job posting alert template', 'subject', 'body'),
  (14, 'recommendation_request', 'Recommendation request template', 'subject', 'body'),
  (15, 'recommendation_follow_up', 'Recommendation follow up template', 'subject', 'body'),
  (16, 'reference_request', 'Reference request template', 'subject', 'body'),
  (17, 'reference_follow_up', 'Reference follow up template', 'subject', 'body'),
  (18, 'ats_scheduling reminder', ' ATS scheduling reminder template', 'subject', 'body'),
  (19, 'ats_workflow_notice', 'ATS workflow notices template, e.g. when a scorecard is marked complete for a candidate, a background check completed, a reference received for a candidate', 'subject', 'body');
...
VALUES(
  ...
  /* type_alias */
  'com_cajobboard.email_message_templates',
  /* table NOTE: No spaces, Joomla! stupidly has this set as a VARCHAR(255) field, how do you add config in that space? */
  '{
    "special":{
      "dbtable":"#__cajobboard_email_message_templates",
      "key":"email_message_template_id",
      "type":"email_message_templates",
      "prefix":"EmailMessageTemplatesTable",
      "config":"array()"
    },
    "common":{

...
/**
 * Data Feeds data model SQL
 *
 * Uses schema https://schema.org/DataFeed
 */
CREATE TABLE IF NOT EXISTS `#__cajobboard_data_feeds` (
  data_feed_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Surrogate primary key', /* FK to #__cajobboard_ucm(id) */
  ...
  note TEXT COMMENT 'A note to save with this data feed in the back-end interface.',
  ...
  description TEXT COMMENT 'Description of the data feed.',
  description__intro VARCHAR(280) COMMENT 'Short description of the item, used for the text shown on browse views.',
  url VARCHAR(2083) COMMENT 'Relative URL of the data feed on this site, if the vendor pulls from this site.',

  /* SCHEMA: https://calligraphic.design/schema/DataFeed */
  data_feed_template INT UNSIGNED COMMENT 'The XML template to apply when generating this data feed.', /* FK to #__cajobboard_data_feed_templates */
  send_dates DATETIME DEFAULT NULL COMMENT 'The date/time at which this data feed should be pushed to the audience, as a  Calligraphic\Cajobboard\Admin\Helper\Enum\DaysOfWeekEnum value.',

  /* SCHEMA: DataFeed */
  audience__vendor BIGINT UNSIGNED COMMENT 'The Vendor that this data feed is for, e.g. Indeed.com', /* FK to #__cajobboard_vendors */
  data_feed_element JSON COMMENT 'A JSON configuration describing Filter parameters to apply for the JobPostings included in the feed, e.g. {"state variable":"job_posting_id","method":"between","from":"1","to":"10"}',

  /* SCHEMA: DataFeed(dataFeedElement) -> DataFeedItem */
  date_created DATETIME DEFAULT NULL COMMENT 'The data and time this data feed was last pushed to the vendor.',
...
/*
 * Create content types for Data Feeds, mapping fields to the UCM standard fields for history feature
 *
...
VALUES(
  ...
  '{
    "common":{
      ...
      "core_catid":"cat_id",
      "core_content_item_id":"data_feed_id",
    ...
    "special":{
      "audience__vendor":"audience__vendor",
      "data_feed_element":"data_feed_element",
      "data_feed_template":"data_feed_template",

...
CREATE TABLE IF NOT EXISTS `#__cajobboard_data_feeds` (
  ...
  note VARCHAR(255) COMMENT 'A note to save with this data feed in the back-end interface.',
  ...
  description TEXT COMMENT 'Short description of the data feed, used for the text shown on social media via shares and search engine results.',
...
/*
Properties from DataFeed:

  DataFeedItem    $dataFeedElement 	An item within in a data feed. Data feeds may have many elements.

Properties from Dataset:

  DataDownload 	  $distribution 	    A downloadable form of this dataset, at a specific location, in a specific format. DataDownload extends MediaObject.


Properties from DataFeedItem, inherits from Thing:

  DateTime 	$dateCreated 	  The date on which the CreativeWork was created or the item was added to a DataFeed.
  DateTime 	$dateDeleted 	  The datetime the item was removed from the DataFeed.
  DateTime 	$dateModified 	The date on which the CreativeWork was most recently modified or when the item's entry was modified within a DataFeed.
  Thing 	  $item 	        An entity represented by an entry in a list or data feed (e.g. an 'artist' in a list of 'artists')’.

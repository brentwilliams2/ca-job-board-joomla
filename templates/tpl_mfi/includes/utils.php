...
global $modules_list, $subnav, $sidenav;
...
foreach ($menu->getMenu() as $item) {
	if ($item->parent_id == 1 && $item->menutype == $menu_name) {
		$toplevel[] = $item->id;
	}
}
...
// get an array of colors Solomon Yark
...
if ($id != 0) {
  ...
  while (true) {
    ...
    if (!isset($item)) break;
    if ($item->parent_id == 1 && $item->level == 1) break;
    $id = $item->parent_id;
  }

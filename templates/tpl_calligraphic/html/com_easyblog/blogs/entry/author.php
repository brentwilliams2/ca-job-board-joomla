...
//MFI - Brent added these two lines to get blogger subscription to work
$bloggerModel = EB::model('Blogger');
$isBloggerSubscribed = $bloggerModel->isBloggerSubscribedEmail($post->author->id, $post->author->user->email);
?>
<div class="eb-entry-author" itemprop="author" itemscope itemtype="http://schema.org/Person">
    <?php /*MFI - Brent removed author heading <h4 class="eb-section-heading reset-heading"><?php echo JText::_('COM_EASYBLOG_ABOUT_THE_AUTHOR');?></h4> */ ?>
    ...
    <div class="eb-entry-author-bio media">
        ...
        <div class="col-cell">
			...
			<?php //MFI - Brent removed nofollow ?>
            <?php if ($post->author->getWebsite() && $this->entryParams->get('post_author_box_website', true)) { ?>
            <div>
                <i class="fa fa-globe"></i>&nbsp; <a href="<?php echo $this->escape($post->author->getWebsite()); ?>" target="_blank" class="author-url"><?php echo $this->escape($post->author->getWebsite()); ?></a>
            </div>
            <?php } ?>

            <div class="eb-entry-author-meta muted">

				<?php //MFI - Brent added this for adding subscribers - given to me by devs */ ?>
				<?php if ($this->config->get('main_bloggersubscription')) { ?>
				<span>
					<a class="<?php echo $isBloggerSubscribed ? 'hide' : ''; ?>" href="javascript:void(0);" data-blog-subscribe data-type="blogger" data-id="<?php echo $post->author->id;?>">Subscribe to <?php echo $post->author->getName();?>'s blogs</a>
					<a class="<?php echo $isBloggerSubscribed ? '' : 'hide'; ?>" href="javascript:void(0);" data-blog-unsubscribe data-type="blogger" data-id="<?php echo $post->author->user->email;?>" data-email="<?php echo $this->my->email; ?>"><?php echo JText::_('COM_EASYBLOG_SUBSCRIPTION_UNSUBSCRIBE_TO_BLOGGER');?></a>
